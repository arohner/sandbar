...
The first three take either the request as an argument or no arguments. The no-argument version can be used in any code that is executed downstream <code>with-security</code> middleware. The var <code>*current-user*</code> is bound to the current user in <code>with-security</code>.

<code>any-role-granted?</code> takes a request and list of roles as arguments or just a list of roles. If passing only a list of roles then it must also be called within <code>with-security</code>.
...
<code>ensure-any-role</code> will first ensure that the <code>*current-user*</code> is authenticated. If not, it will raise an authentication error. It will then ensure that the <code>*current-user*</code> is in one of the roles. If not, it will raise an access error.
...
* Make it easy for users of this lib to create a login form that may be embedded within a page.
...
Every application that I create needs to have some kind of user management GUI. The goal here is to package this up into a library that I can just include and configure and never have to worry about again. This is also partially implemented.
