...
This branch currently does not work. I am in the middle of migrating to Compojure 0.4 an and Ring 2.0.

Sandbar is a set of libraries for web applications which is designed to be used with "Compojure":http://github.com/weavejester/compojure and "Ring":http://github.com/mmcgrana/ring. It contains (or will contain) code to help with: 
...
<code>ensure-any-role</code> will first ensure that the <code>*current-user*</code> is authenticated. If not it will raise an authentication error. It will then ensure that the <code>*current-user*</code> is in one of the roles. If not then it will raise an access error.
